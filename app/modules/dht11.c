...
void ICACHE_FLASH_ATTR DHT11_Signal_Output(u8 Value_Vol)
{
	...
	GPIO_OUTPUT_SET(GPIO_ID_PIN(0),Value_Vol);				// IO5设为输出=X
}
//===========================================================================
...
{
	PIN_FUNC_SELECT(PERIPHS_IO_MUX_GPIO0_U,	FUNC_GPIO0);	// GPIO5设为IO口
	GPIO_DIS_OUTPUT(GPIO_ID_PIN(0));		// GPIO5失能输出(输入)
}
//===========================================================================
...
u8 ICACHE_FLASH_ATTR DHT11_Start_Signal_JX(void)
{
	...
	// IO5拉低(25ms)：起始信号
	//---------------------------------------------
	GPIO_OUTPUT_SET(GPIO_ID_PIN(0),0);	// IO5 = 0
	Dht11_delay_ms(25);

	// IO5抬高【注：起始信号结束后的约13us，DHT11开始输出信号】
	//---------------------------------------------------------
	GPIO_OUTPUT_SET(GPIO_ID_PIN(0),1);	// IO5 = 1
	os_delay_us(5);		// 延时5us
	...
	// 等待响应信号的低电平【最迟等50us】
	//-------------------------------------------------------------
	while( GPIO_INPUT_GET(GPIO_ID_PIN(0))==1 && C_delay_time<50 )
	{
		os_delay_us(1);		// 1us计时
		C_delay_time++;
	}
	...
	//-------------------------------------------------------------
	while( GPIO_INPUT_GET(GPIO_ID_PIN(0))==0 && C_delay_time<170 )
	{
		os_delay_us(1);
		C_delay_time++;	// 低电平时长
	}
...
u8 ICACHE_FLASH_ATTR DHT11_Read_Bit(void)
{
	...
	// 等待响应信息的低电平【最迟等150us】
	//-------------------------------------------------------------
	while( GPIO_INPUT_GET(GPIO_ID_PIN(0))==1 && C_delay_time<150 )
	{
		os_delay_us(1);		// 1us计时
		C_delay_time++;
	}
	...
	// 数据位的低电平时长计时【最多200us】
	//-------------------------------------------------------------
	while( GPIO_INPUT_GET(GPIO_ID_PIN(0))==0 && C_delay_time<120 )
	{
		os_delay_us(1);
		C_delay_time++;	// 低电平时长
	}
	...
	// 如果此时信号线电平==0 => 数据=="0"
	//-------------------------------------
	return GPIO_INPUT_GET(GPIO_ID_PIN(0));
}
//======================================================================
...
u8 ICACHE_FLASH_ATTR DHT11_Read_Data_Complete(void)
{
	...
	if(DHT11_Start_Signal_JX() == 0)	// DHT11：输出起始信号－＞接收响应信号
	{
		...
		//-----------------------------------------------------------
		while(GPIO_INPUT_GET(GPIO_ID_PIN(0))==1 && C_delay_time<100)
		{
			os_delay_us(1);		// 1us计时
			C_delay_time++;
		}
		...
		// 结束信号的低电平时长计时
		//-----------------------------------------------------------
		while(GPIO_INPUT_GET(GPIO_ID_PIN(0))==0 && C_delay_time<100)
		{
			os_delay_us(1);		// 1us计时
			C_delay_time++;
		}
