...
//--------------------------------------------------------------------------------------------------------------------------------------
#define MQTT_HOST			"mqtt.acbd.top" 		// MQTT服务端域名/IP地址	// the IP address or domain name of your MQTT server or MQTT broker ,such as "mqtt.yourdomain.com"
#define MQTT_PORT       	1883    										// 网络连接端口号			// the listening port of your MQTT server or MQTT broker
char MQTT_CLIENT_ID[10];   		// 官方例程中是"Device_ID"		// 客户端标识符				// the ID of yourself, any string is OK,client would use this ID register itself to MQTT server
char MQTT_USER[20];        			// MQTT用户名				// your MQTT login name, if MQTT server allow anonymous login,any string is OK, otherwise, please input valid login name which you had registered
#define MQTT_PASS        	"WWYksu2nyuft+ycYYOw11MBo1fTQvz1eI8LpeC050nk=" 	// MQTT密码					// you MQTT login password, same as above

#define STA_SSID 			"427"    	// WIFI名称					// your AP/router SSID to config your device networking
#define STA_PASS 			"wo101010" 	// WIFI密码					// your AP/router password
